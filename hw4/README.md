...
**Внутренняя реализация БД** определяет, как именно данных хранятся. Ключевые компоненты реализации: 
_узел_, _сессия_, _запрос_ и _путь_. **Драйвера** предоставляют API, с помощью которого тут можно использовать разные БД:
* _Apache Derby_
* _MySQL_
* _PostgreSQL_
